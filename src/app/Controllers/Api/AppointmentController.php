...
class AppointmentController extends ApiController
{
    ...
    public function deleteRecord(Request $request, Response $response, Array $args) : Response 
    {
        ...
        // log request along with hash
        $this->container->logger->info('Cancel Appointment Request', ['hash' => $hash, 'request', $request->getParams()]);
        ...
        // if validation fails return error
        if ( $validation->failed() ) {
            $this->container->logger->error('Cancel Appointment Failed Validation', [
                'hash' => $hash, 
                'errors' => $validation->errors()
                ]);
            return $response->withJson([
        ...
        $Slot = new Slot($request->getParams());
        $status = $Slot->cancelAppointment();

        // status code will be 200 if sucessful
        if ($status['code'] == 200) {
            $this->container->logger->info('Cancel Appointment Response', ['hash' => $hash, 'response' => $status]);
        } else {
            $this->container->logger->error('Cancel Appointment Response', ['hash' => $hash, 'response' => $status]);
        }
